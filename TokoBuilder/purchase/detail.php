...
if (!isset($_GET['nota_id']) or !isset($_GET['nama_supplier'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $order_restocks = get_order_restock_by_nota_id($con, $nota_id);
    ...
    update_delivered_by_order_restock_id($con, $data['updates']);
    
    // TODO: Update hutang.
    update_hutang_by_nota_id($con, $nota_id, $data['paidAmount']);
...
$order_restock = get_order_restock_by_nota_id($con, $nota_id);
                            ...
                            <tbody class="text-gray-700">
                                ...
                                ?>
                                    <tr class="border-t" data-order-id="<?= $row['order_restock_id']; ?>">
                                        ...
                                        <td class="py-2">
                                            <button class="return-button bg-purple-600 text-white px-2 py-1 rounded-full" onclick="openReturnModal()">Return</button>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
...
<script>
    ...
    function saveModal() {
        const inputText = $("#inputText").val();  // Get the value from the input field
        ...
        const deliveredCell = row.find("td:nth-child(5)");  // Update a specific column (e.g., Column 5)
        deliveredCell.text(inputText); 

        $("#inputText").val("");  // Clear the input field
        closeReturnModal();  // Close the modal
}
    function saveChanges() {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ updates: updates, paidAmount: $('#paidAmount').val() }),
            complete: function(xhr) {
                alert(JSON.parse(xhr.responseText).message);
                location.reload();
            }
