...
if(isset($_POST['add_hutang'])){
    ...
    $sql = "INSERT INTO piutang (customer_id,receivable_amount,deadline) VALUES ($supplier, $total, '$due_date')";
    query_no_return($sql);
    checkPiutang();
  }
  if(isset($_POST['delete_hutang'])){
    $hutang_id = $_POST['delete_id'];
    $sql = "DELETE FROM piutang WHERE receivable_id = $hutang_id";
    query_no_return($sql);
  }
  if(isset($_POST['change_hutang'])){
    ...
    $due_date = $_POST['due_date'];
    query_no_return("UPDATE piutang SET receivable_amount = $hutang_total, deadline = '$due_date' WHERE receivable_id = $id");
    checkPiutang();
  }
$piutangs = query("SELECT * FROM piutang JOIN customer ON piutang.customer_id = customer.customer_id");
$custs = query("SELECT * FROM customer");
?>
...
<body>
  ...
  <main class="container mx-auto bg-white p-6 rounded shadow w-3/4 mt-4">
    <h2 class="text-2xl font-bold mb-4">Add Piutang</h2>
    <form action="" method="post">
        <div class="mb-4">
            ...
            <button class="btn bg-purple-700 text-white dropdown-toggle supplier_selector" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false">
                Select Customer
            </button>
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
              <?php foreach ($custs as $cust){?>
                <li><a class="dropdown-item main-dropdown" href="#" data-value="<?php echo $cust['customer_id']; ?>" data-name="<?php echo $cust['customer_name']; ?>"><?php echo $cust['customer_name']; ?></a></li>
              <?php } ?>
            </ul>
        </div>
        <div class="mb-4">
        ...
        <button type="submit" name="add_hutang" class="bg-purple-700 text-white px-4 py-2 rounded">Add Piutang</button>
    </form>
  </main>
  <br>
  <table class="table-auto mx-auto bg-white rounded shadow w-3/4">
      ...
      <tbody>
        ...
        foreach ($piutangs as $row) {
          echo "<tr>
                  <td class='border px-4 py-2'>{$row['receivable_id']}</td>
                  <td class='border px-4 py-2'>{$row['customer_name']}</td>
                  <td class='border px-4 py-2'>{$row['receivable_amount']}</td>
                  <td class='border px-4 py-2'>{$row['deadline']}</td>
                  <td class='hidden'>{$row['customer_id']}</td>
                  <td class='border px-4 py-2'>
                    <div class='flex space-x-2'>
                    <button 
                        onclick=\"openModal('{$row['receivable_id']}', '{$row['customer_name']}', '{$row['receivable_amount']}', '{$row['deadline']}', '{$row['customer_name']}')
                       \"
                        class='bg-green-500 text-white px-4 py-2 rounded' id='editButton'>Edit
                    </button>
                      <form action='' method='post'>
                        <input type='hidden' name='delete_id' value='{$row['receivable_id']}' />
                        <button type='submit' name='delete_hutang' class='bg-red-500 text-white px-4 py-2 rounded'>Delete</button>
                      </form>
                    </div>
                  </td>
                </tr>";
    ...
    <div class="bg-white p-6 rounded shadow-lg w-96">
      ...
      <form action="" method="post">
        <div class="mb-4">
            ...
            <label for="supplier" class="block text-gray-700 font-bold mb-2">Customer:</label>
            <button class="btn bg-purple-700 text-white dropdown-toggle supplier_selector" type="button" id="edit-dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                Select Supplier
            </button>
            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        ...
        <div class="flex justify-end space-x-4">
          ...
          <button type="submit" name="change_hutang" class="bg-purple-700 text-white px-4 py-2 rounded">Change Hutang</button>
        </div>
    </form>
    </div>
    <script>
      ...
      $('.main-dropdown').click(function(e) {
        ...
        document.getElementById('selected_supplier_id').value = supplierId;
