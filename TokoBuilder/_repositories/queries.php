...
function get_all_suppliers(mysqli $conn): array {
    $result = $conn->query("SELECT supplier_id, nama_supplier FROM Supplier");
...
function update_hutang_by_nota_id(mysqli $conn, int $nota_id, float $paid_amount): void {
    $query = "UPDATE Hutang SET hutang_total = hutang_total - ? WHERE nota_id = ?";
...
function create_sales_record(mysqli $conn, string $tanggal, array $products): void {
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO Catatan_Penjualan (transaction_date) VALUES (?)");
        $stmt->execute([$tanggal]);
        
        $catatan_penjualan_id = $conn->insert_id;
        
        $detail_stmt = $conn->prepare(
            "INSERT INTO Penjualan_Detail (produk_id, catatan_id, penjualan_detail_qty) 
             VALUES (?, ?, ?)"
        );
...
function get_sales_summary(mysqli $conn): array {
    ...
    FROM Catatan_Penjualan cp
    JOIN Penjualan_Detail pd ON cp.catatan_id = pd.catatan_id
    JOIN Produk p ON pd.produk_id = p.produk_id
    GROUP BY cp.catatan_id, cp.transaction_date
    ORDER BY cp.catatan_id
