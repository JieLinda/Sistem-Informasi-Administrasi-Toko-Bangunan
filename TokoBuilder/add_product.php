...
if (isset($_POST["confirm"])){
    ...
    if ( isset($_POST['nama_produk']) && isset($_POST['stok']) && isset($_POST['harga_jual']) ) {

        $nama = $_POST['nama_produk'];
        $stok = $_POST['stok'];
        $harga = $_POST['harga_jual'];

        $stmt = $con->prepare("INSERT INTO produk VALUES('',?, ?, ?, ?)");
        $stmt->bind_param("ssss",$nama,$stok,$harga,$username);
        $stmt->execute();
        $stmt->close();
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <form action="add_product.php" method="POST">
                            <div class="mb-4">
                                ...
                                <input class="border border-gray-300 rounded-lg px-3 py-2 w-full" type="text" id="nama_produk" name="nama_produk" required />
                            </div>
                            <div class="mb-4">
                                <label class="block text-gray-700 text-sm font-bold mb-2" for="stok">
                                    Stok
                                </label>
                                <input class="border border-gray-300 rounded-lg px-3 py-2 w-full" type="number" id="stok" name="stok" min="1" required />
                            </div>
                            <div class="mb-4">
                                <label class="block text-gray-700 text-sm font-bold mb-2" for="harga_jual">
                                    Harga Jual
                                </label>
                                <input class="border border-gray-300 rounded-lg px-3 py-2 w-full" type="text" id="harga_jual" name="harga_jual" required />
                            </div>
                            <div class="flex justify-between">
                                <button type="button" class="bg-gray-400 text-white px-4 py-2 rounded-lg" onclick="window.location.href='product.php'">
                                    Cancel
                                </button>
